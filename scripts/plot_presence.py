...
presence["group"] = presence.loc[:, "group"].apply(
    lambda x: round(int(x) * 30 / 7, 0) if rgx.search(x) else x
)

presence_summer_all = pd.concat([presence, presence_monthly]).reset_index()
presence_summer_all["group"] = presence_summer_all["group"].astype("str")

presence_summer_all = (
    presence_summer_all.groupby("group")["count"]
    .sum()
    .reset_index()
    .assign(count_population=lambda x: x["count"] * PERCENT / n)
    .sort_values("count_population")
)

presence_summer_all["group"] = presence_summer_all["group"].astype("str")

fig = plot_barhplot(
    df=presence_summer_all,
    x="group",
    y="count_population",
    labels=False,
    percenteges=True,
...
fig.tight_layout()
fig.savefig(PNG / "per_monthly-all-presence-summer.png")
if __name__ != "__main__":
    plt.show()

presence_summer_monthly_all = presence_summer_all.set_index("group")
# %%
...
presence_winter_all = pd.concat([presence, presence_monthly]).reset_index()
presence_winter_all["group"] = presence_winter_all["group"].astype("str")

presence_winter_all = (
    presence_winter_all.groupby("group")["count"]
    .sum()
    .reset_index()
    .assign(count_population=lambda x: x["count"] * PERCENT / n)
    .sort_values("count_population")
...
fig.tight_layout()
fig.savefig(PNG / "per_monthly-all-presence-zimowy.png")
if __name__ != "__main__":
    plt.show()

presence_winter_monthly_all = presence_winter_all.set_index("group")
# %%
for _, role in ndf.groupby("role"):
    ...
    fig.tight_layout()
    fig.savefig(PNG / f"per_{_}_monthly-all-presence-summer.png")
    if __name__ != "__main__":
        plt.show()

    presence_summer_monthly_all = presence_summer_monthly_all.join(
        presence_summer_all.set_index("group"), how="outer", rsuffix=f" {_}"
    )
    ...
    fig.tight_layout()
    fig.savefig(PNG / f"per_{_}_monthly-all-presence-winter.png")
    if __name__ != "__main__":
        plt.show()

    presence_winter_monthly_all = presence_winter_monthly_all.join(
        presence_winter_all.set_index("group"), how="outer", rsuffix=f" {_}"
    )
...
# %%
## PRESENCE SUMMER ALL MONTHLY
(
    presence_summer_monthly_all.rename(
        columns=lambda x: x.replace("count_population", "Procent").replace(
            "count", "Liczebność ważona"
        )
    )
    ...
    .rename(columns={"group": "Liczba dni na UW"})
    .fillna(0)
    .to_excel(EXCEL / "P4bb.xlsx", index=False)
)
# %%
## PRESENCE WINTER WEEKLY
(
...
# %%
## PRESENCE WINTER ALL MONTHLY
(
    presence_winter_monthly_all.rename(
        columns=lambda x: x.replace("count_population", "Procent").replace(
            "count", "Liczebność ważona"
        )
    )
